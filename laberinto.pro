...
%% Caso que procesa la lectura cuando el caracter es salto de linea
...
%% Caso que verifica si el pto es parte de la solucion.
...
%% Caso que verifica si el pto es parte de los ptos abiertos originales.
procesar_escritura(X,Y,Solucion,Original,M,N):- 
...
%% Caso en que el pto es un pto cerrao/pared.
procesar_escritura(X,Y,Solucion,Original,M,N):- 
	write('#'),
	siguiente(X,Y,XN,YN,M,N),
...
%% Caso en el que se imrpimio un elemento de una fila y se procede al siguiente.
